    ...
    public class XMLRepositoryBase<TContext, TEntity, TKey> : IRepository<TEntity, TKey> where TContext : XMLSet<TEntity> where TEntity : class
    {
        ...
        public bool Delete(TKey id) 
        {
            try 
            {
                ...
                items.Remove(items.First(f => f.TraineeID.Equals(id)));
        ...
        public TEntity Get(TKey id)
        {
            ...
            {
                List<IEntity<TKey>> items = m_context.Data as List<IEntity<TKey>>; 
                return items.FirstOrDefault(f => f.TraineeID.Equals(id)) as TEntity;
            }
            catch (Exception) { return null; 
            }
        }

        public ICollection<TEntity> GetAll() { return m_context.Data;
        ...
        public bool Remove(TKey id) { return Delete(id);
        ...
        { 
...

            {
                ...
                List<IEntity<TKey>> items = m_context.Data as List<IEntity<TKey>>;
                items.Remove(items.FirstOrDefault(f => f.TraineeID.Equals(imodel.TraineeID))); 
                items.Add(imodel);
                m_context.Data = items as ICollection<TEntity>; 
                return true;
            }
